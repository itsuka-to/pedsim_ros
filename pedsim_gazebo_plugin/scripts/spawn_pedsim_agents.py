...
def actor_poses_callback(actors):
    ...
    for actor in actors.agent_states:
        ...
        spawn_entity(node, xml_string, model_pose)
...
def spawn_entity(node, entity_xml, initial_pose, timeout=DEFAULT_TIMEOUT):
    if timeout < 0:
        node.get_logger().error('spawn_entity timeout must be greater than zero')
        return False
    node.get_logger().info(
        'Waiting for service %s/spawn_entity, timeout = %.f' % (
            node.args.gazebo_namespace, timeout))
    node.get_logger().info('Waiting for service %s/spawn_entity' % node.args.gazebo_namespace)
    client = node.create_client(SpawnEntity, '%s/spawn_entity' % node.args.gazebo_namespace)
    if client.wait_for_service(timeout_sec=timeout):
        req = SpawnEntity.Request()
        req.name = node.args.entity
        req.xml = str(entity_xml, 'utf-8')
        req.robot_namespace = node.args.robot_namespace
        req.initial_pose = initial_pose
        req.reference_frame = node.args.reference_frame
        node.get_logger().info('Calling service %s/spawn_entity' % node.args.gazebo_namespace)
